<#  
    ...
    .Notes  
        ...
        Version        : 1.0 - 2021/03/19 - Initial release  
                         ...
                         1.2 - 2021/09/27 - Escape strings xml compatible
...
$cleanAppNameXmlCompatible = ConvertTo-XmlCompatibleString $cleanAppName;
...
$omaConfigFile = [PSCustomObject]@{
    ...
    "description" = ""
    "value" = "<enabled/>
<data id=`"$cleanAppNameXmlCompatible-$cleanAppPolicyNameXmlCompatible-File-path`" value=`"$outFilePathXmlCompatible`"/>
<data id=`"$cleanAppNameXmlCompatible-$cleanAppPolicyNameXmlCompatible-File-operation`" value=`"create`"/>
<data id=`"$cleanAppNameXmlCompatible-$cleanAppPolicyNameXmlCompatible-File-encoding`" value=`"$encodingXmlCompatible`"/>";
...
ForEach ( $sectionname in $iniSectionKeys ) {
    ...
    $policyValueXmlCompatible = ConvertTo-XmlCompatibleString $value;
    $policyOperationXmlCompatible = ConvertTo-XmlCompatibleString $Operation
    $policySectionNameXmlCompatible = ConvertTo-XmlCompatibleString $sectionname;
    $policyKeyNameNameXmlCompatible = ConvertTo-XmlCompatibleString $keyname;
...
<data id=`"$policyNameXmlCompatible-section`" value=`"$policySectionNameXmlCompatible`"/>
<data id=`"$policyNameXmlCompatible-key`" value=`"$policyKeyNameNameXmlCompatible`"/>"
...
# Set-AuthenticodeSignature "$env:userprofile\GitHub\PolicyApplicator-for-Microsoft-Intune\Code\Convert-iniToOMACSVTemplate.ps1" @(Get-ChildItem cert:\CurrentUser\My -codesigning)[0] -TimestampServer http://time.certum.pl
# SIG # Begin signature block
# MIIk+QYJKoZIhvcNAQcCoIIk6jCCJOYCAQExCzAJBgUrDgMCGgUAMGkGCisGAQQB
# gjcCAQSgWzBZMDQGCisGAQQBgjcCAR4wJgIDAQAABBAfzDtgWUsITrck0sYpfvNR
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUEIFNx1JmRNIeu6DuU1VhdnLh
# JEeggh4pMIIFCTCCA/GgAwIBAgIQDapMmE8NUKJDb44cpXT3cDANBgkqhkiG9w0B
...
# DAYKKwYBBAGCNwIBFTAjBgkqhkiG9w0BCQQxFgQUCw/cDpfuwive/GuGnip0QFMe
# coIwDQYJKoZIhvcNAQEBBQAEggEAFbzFYem196zaRi6fmRLAmeRbAVjnlLxDuxNd
# F8Jj605aAQlqMiqW1PUI0NNT14E2bjUvnUiEN/eNyUhGnYARd9uySOGBfJV2OQSf
# MeH2zpJ40dfvCl/Hays/PdviktrfRG3dFqbd/kYXiDW9dUja5Vnk2rtk5MLhUdhK
# D3qrFYwpOmZnMRHjVR2r9t5SpnBM49juyOzPuArLi4eSOU13thGw1Y3EbOPxAy++
# /Vp7jOu3xhbie4hRSvklCzFoWqEvBrwkNACYQKpJIIwOvArnZ5+FYitC9JFcq0w5
# 3osArFqypvVq2qRf4fvIkSRUeyLxk7af2P0HqFcG1lrI4l0ZfqGCBAQwggQABgkq
...
# MjEwOTI3MTAzNjIzWjA3BgsqhkiG9w0BCRACLzEoMCYwJDAiBCAbWb/o5XcrrPZD
# u3mstI6BWHhPIcVUrhNHbToaPgXF0zA/BgkqhkiG9w0BCQQxMgQwIouxnj/Eqma9
# O1W4LGPfljSlxQ8aLqQl8j4e6r0WpzG2K9zQkBE4AVTZsr48/9mUMIGgBgsqhkiG
...
# 4nviDjJgjkv0qDANBgkqhkiG9w0BAQEFAASCAgAQqhTdF0A1f1ny+wxRj/TstRku
# l6KFNt1JP4YhefYgXeDYwtrL4Gwfz+OCE8BPpgqWj3TcWFdIhtCo+sIg7O6o0OBK
# nILwxj8T1WZo8Tc2llDc7kbVKRWSO/I+H7NHdKe1pZyWCQc1YFBFM+yjkBs+vY1H
# PCGjacBZH7o3x/wH5+TSYIC9798oBT0xXY2R66YgDX92RTGQxTaDBNLKjckKc2vf
# /Pr0Z8Ow22KGZ4J7pTYREGAc6bBIe3kHTJaNmasnSIebSZzoEFb4f0olblClZZA+
# KpNHlTsH+nxci0Ibger9WCfnCygGtQkGBJ4ebXdXJ6Y/pI9dw9Y+SmAb9+6tzEGQ
# Zf88uuitBCARJMex+NiqF5MjxjElDH6W1zUYgAeZfpYTc3Mvkon/8MBJofwfHZuZ
# wvv/zRX49oT5Asy5AAC7D8DPJZb5YiRuFc+QQL8fhE9s/EbWRpGQ/Prymvn/fR5M
# tq5SRWV5AlzS3fwimQ+F45E/Y8Cp1Bewy405McBMfCqfB1htEzG2cY9x0p+8YvFq
# l9AddilaqISJKsBXrNFRGc+3f3ULwH1XD1tP8bEsqL9kIEFPVg5HmBgBOW4JmuC0
# m2iI9xmBGqW61ZTZbU+sgSGXreX6nJWeEfUrs0C30XzI0RAxZIqsuejkEMnuqdnI
# r5ojUiAIpmo5VfrvIg==
